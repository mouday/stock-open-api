...
from stock_open_api.utils import request_util, table_util
...
def get_company_info(stock_code):
    ...
    for item in sel.css('.company-sheet .sheet-item'):
        key = item.css('span::text').extract_first('')
        value = item.css('strong::attr(title)').extract_first('')
    ...
    for key, value in zip(sel.css('.company-total .lc-title span::text').extract(), sel.css('.company-total .lc-main')):
        ...
        if key in ['公司简介', '主营业务', '经营范围']:
            ...
            data[key] = table_util.parse_table(value)
        elif key == '高管介绍':
            data[key] = parse_introduce_table(value)
        elif key == '新闻资讯':
